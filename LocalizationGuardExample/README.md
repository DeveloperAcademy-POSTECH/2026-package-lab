...
## 6-1. 한계점 1: 언어별 문자 체계를 이해해야 함

처음에는 한국어 원문 문자열을 기준으로 만들었습니다.
...
현재 지원하는 원문 언어는 한국어(`ko`)와 일본어(`ja`)입니다.

```swift
{
  "sourceLanguages": ["ko", "ja"]
}
```

처럼 두 언어를 함께 사용하는 프로젝트도 설정할 수 있습니다.

새로운 언어를 추가할 때마다 그 언어의 문자 체계와 실제 문자열 작성 방식을 이해해야 합니다. 특히 여러 문자 체계를 함께 사용하는 언어는 탐지 범위를 정하는 과정이 어렵겠조?ㅠ..ㅠ
...
그래서 현재는 **번역을 자동으로 고쳐 주는 도구**보다는 **번역이 필요할 수 있는 위치를 빠르게 알려주는 도구**에 집중했습니다.

AI 자동 번역은 빌드 과정이 아닌 별도의 명령이나 기능으로 제공하는 방식은 나중에 고민해 볼 수 있겠지만은??!!! 지금은 구현하지 않으려고 합니다. 제 관심 영역이 아니에여ㅎㅅㅎ😜
...
- `Text`, `Button`, `Label`처럼 화면에 표시되는 SwiftUI API를 우선 검사하기
- `print`, `debugPrint` 같은 함수는 제외하기
- 개발자가 제외할 경로와 함수를 설정할 수 있게 하기
- 필요하다면 검사할 API를 직접 설정할 수 있게 하기

영어 원문도 놓치지 않으면서 불필요한 경고를 줄이는 방법을 앞으로 더 고민해 보고 싶습니다.

이 문제에 관심이 있거나 더 좋은 방법이 떠오르시는 분은!!!!! 저랑 같이 개발해 봐요🫪
