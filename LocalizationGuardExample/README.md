...
현재 일반 문자열 탐지는 원문 언어의 Unicode 범위를 기준으로 동작합니다. 한국어는 한글 범위가 비교적 명확하지만, 일본어는 히라가나·가타카나·한자가 함께 사용되어 고려할 범위가 더 많습니다.

새로운 원문 언어를 지원하려면 그 언어의 문자 체계와 코드 안에서 자주 쓰이는 예외를 함께 이해해야 합니다.

Unicode 범위를 확인할 때는 [Unicode Character Code Charts](https://www.unicode.org/charts/)를 참고했습니다.

## 6-2. 한계점 2: 번역 품질까지는 판단할 수 없음

LocalizationGuard는 String Catalog에 키가 있는지, 필요한 언어의 번역이 있는지, 번역값이 비어 있지 않은지를 확인합니다.

하지만 번역 문장이 자연스러운지, 화면 문맥에 맞는지, 복수형 표현이 적절한지, 번역 검토가 끝났는지까지 판단하지는 않습니다.

AI를 연결해 자동 번역이나 품질 검토를 돕는 방법도 생각해 볼 수 있습니다. 다만 빌드 과정에서 모든 문장을 처리하면 빌드가 느려질 수 있으므로, 현재는 번역을 평가하기보다 누락 가능성을 알려주는 역할에 집중하고 있습니다.

## 6-3. 한계점 3: 현재 경고는 오탐이 될 수 있음
